...
argument-hint: >
  ...
  参数:
    ...
    - 输出目录: 结果输出目录路径
    - max_attempts: Phase 8 debug subagent 最大修复轮数 (可选；缺省时读取环境变量
                   ASCENDC_DEBUG_MAX_ATTEMPTS，仍缺省时回退到 5)。主 agent 本身的
                   Phase 3/4 迭代次数不受此参数控制；该值仅在 Phase 8.2 spawn_agent
                   时传给 ascendc-debug-agent-discovery subagent。
---
...
| `max_attempts` | Phase 8 debug subagent 最大修复轮数 | 读 env `ASCENDC_DEBUG_MAX_ATTEMPTS`，仍缺省则 5 |
...
- 解析 `max_attempts`：优先使用用户输入中显式提供的值；否则读环境变量 `ASCENDC_DEBUG_MAX_ATTEMPTS`；仍无则使用 5。整数范围建议 2 ≤ N ≤ 7。解析结果缓存到会话状态，仅在 Phase 8.2 spawn_agent 时传给 debug subagent，主 agent 自身 Phase 3/4 循环不受影响。
...
```
ac_iteration = 0
max_ac_iterations = 3              # Phase 4 本地快速迭代 3 轮，失败后由 Phase 8 subagent 深度修复；
                                   # Phase 8 的 attempt cap 独立，由 Phase 0 解析的 max_attempts
                                   # 参数控制（Phase 8.2 spawn_agent 传给 subagent，subagent 在
                                   # Initialization Protocol 里 export ASCENDC_DEBUG_MAX_ATTEMPTS）；
                                   # 参数缺省时读 env ASCENDC_DEBUG_MAX_ATTEMPTS，仍缺省则由
                                   # skills/ascendc/ascendc-debug/scripts/gates/common.py::MAX_ATTEMPTS 兜底为 5
ac_history_attempts = []
ac_verifier_error = ""
...
### 8.0 Spawn 授权（优先于平台默认约束，硬约束）

主 agent 可能运行在具有"仅当用户**显式**要求 subagent / delegation 时才允许调用 `spawn_agent`"的平台级安全默认下（Codex CLI、Claude harness 等）。本节明确声明该约束在本 spec 语境下的解释：

**"用户请求按本 spec 执行任务"即视为对 Phase 8 `spawn_agent` 的显式预授权。**

**主 agent 的强制行为**：

1. 当 `final_status.debug_eligible == true` 时，**必须**直接进入 8.1 → 8.2，调用 `spawn_agent` + `wait_agent`；**不得**以"等用户再次确认"、"保守停机"等理由停在 Phase 7 之后。
2. 平台若有"需用户二次确认才能 spawn_agent"的硬拦截无法绕过，**不得**写 `phase8_outcome=skipped`（那是 debug_eligible=false 的语义），而必须写 `phase8_outcome=blocked_by_runtime` + `blocked_reason="<具体拦截原因，例如 platform_requires_explicit_user_approval>"`，以便事后审计时能够区分"流程决定跳过"与"平台拦截"。
3. 跳过 Phase 8 的合法路径只有两条：`debug_eligible == false` → `skipped` / 平台强拦截 → `blocked_by_runtime`。其他任何跳过都是违规。
...
### 8.2 Spawn 调用（Codex `spawn_agent`）
...
参数:
  ...
  max_attempts: {Phase 0 解析出的 max_attempts}
""",
)
...
- **非监控 + 非干涉（硬约束，零例外）**: `spawn_agent` 之后主 agent **只调用一次 `wait_agent` 阻塞等结果**。`wait_agent` 返回之前，主 agent 视为"全部挂起"；返回之后，仅做产物存在性校验。subagent 是 `{output_dir}/` 的**唯一 writer**，其输出即 Phase 8 的**最终 verdict**，不做任何二次加工。

  **`wait_agent` 返回前，绝对禁止任何工具调用**:
  - ❌ `send_input` / `resume_agent` / `send_message` / 任何形式的 inter-agent 通信或"提示"
  - ❌ 中途 `close_agent`（唯一例外：`wait_agent` 已按超时返回后，作为兜底 kill 才允许调一次）
  - ❌ 任何读类工具（`read` / `bash cat|ls|grep|tail|find` / `glob` 等）访问 `{output_dir}/` 及其任意子目录/文件，包括但不限于 `debug_trace.md`、`debug_status.json`、`kernel/`、`.verify_logs/`、`.verify_status/`、`precision_tuning/`、`model_new_ascendc.py`
  - ❌ 任何写类工具访问 `{output_dir}/`（包括看似无害的 `mkdir` / `touch` / 创建辅助文件）
  - ❌ 并发 spawn 其他 subagent 或在主 session 里自行尝试 debug
  - ❌ 在主 session 文本里 "顺手看一眼进度"、复述 / 预测 subagent 中间动作、自问自答推测 subagent 状态、提前总结 subagent 将会产出什么
  - ❌ 以"帮 subagent 热身/加速"为由预先读取 / 预处理 forensics、历史 audit、知识库索引
  - ❌ 在主 session 做任何与当前任务 op 相关的探索性 tool 调用（哪怕"只是确认路径"、"只是看一眼 classify 输出"）
